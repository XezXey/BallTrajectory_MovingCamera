...
def get_selected_cols():
  ...
  if 'azimuth' in args.selected_features:
    features_cols.append('azimuth')
  if 'elevation' in args.selected_features:
    features_cols.append('elevation')
  if 'intrinsic' in args.selected_features:
    features_cols.append('intrinsic')
  if 'extrinsic' in args.selected_features:
    features_cols.append('extrinsic')
  ...
  for tmp in ['intersect', 'ray', 'cam']:
    for axis in ['x', 'y', 'z']:
      position_cols.append('{}_{}'.format(tmp, axis))
...
def txt_to_mat(txt):
  ...
  mat = np.array([np.array(t).astype(np.float64) for t in txt[:-1]])
  return mat 
...
def computeDisplacement(trajectory_split, trajectory_type):
  ...
  for traj_type in trajectory_type:
    ...
    # Second hstack(extend columns) with (All columns, ['end_of_trajectory', force, etc] column) 
    for i in range(len(trajectory_split[traj_type])):
      trajectory_split[traj_type][i]['intrinsic'] = list(map(txt_to_mat, trajectory_split[traj_type][i]['intrinsic'].values))
      trajectory_split[traj_type][i]['extrinsic'] = list(map(txt_to_mat, trajectory_split[traj_type][i]['extrinsic'].values))
      trajectory_split[traj_type][i]['extrinsic_inv'] = list(map(np.linalg.inv, trajectory_split[traj_type][i]['extrinsic']))
      #trajectory_split[traj_type][i]['cx'] = [trajectory_split[traj_type][i]['extrinsic_inv'][j] for j in range(trajectory_split[traj_type])
      trajectory_npy[traj_type][i] = np.hstack((trajectory_split[traj_type][i][position_cols].values,
    ...
    plt.legend()
    plt.savefig('./etc/uvd_prep.png')



    plt.clf()
    plt.plot(np.arange(temp.shape[0]), temp[:, [0]], '-or', label='x')
    plt.plot(np.arange(temp.shape[0]), temp[:, [1]], '-og', label='y')
    plt.plot(np.arange(temp.shape[0]), temp[:, [2]], '-ob', label='z')
    plt.plot(np.arange(temp.shape[0]), temp[:, [12]], '-or', label='eot')
    plt.legend()
    plt.savefig('./etc/xyz_prep.png')
    plt.show()
    ...
    trajectory_npy[traj_type] = remove_bad_trajectory(trajectory=trajectory_npy[traj_type], traj_type=traj_type)
  return trajectory_npy

def remove_bad_trajectory(trajectory, traj_type):
  ...
  for idx in range(trajectory.shape[0]):
    traj_temp = trajectory[idx][:, :]
    is_bad = (np.any(traj_temp[:, 1] <= -1) or np.any(traj_temp[:, 3] < 0) or np.any(traj_temp[:, 3] > width)
        or np.any(traj_temp[:, 4] < 0) or np.any(traj_temp[:, 4] > height))
    ...
    #extrinsic = traj_temp[:, -1]
    #cx = [extrinsic[i][0, -1] for i in range(extrinsic.shape[0])]
    #cy = [extrinsic[i][1, -1] for i in range(extrinsic.shape[0])]
    #cz = [extrinsic[i][2, -1] for i in range(extrinsic.shape[0])]

    if 'hw' in args.selected_features:
      is_bad = is_bad or np.any(trajectory[idx][:, 17])
      trajectory[idx] = np.delete(trajectory[idx], [17], axis=1)
      #print(trajectory[idx][:, 8])
      #if np.any(trajectory[idx][:, 8]):
      #  plt.plot(traj_temp[:, 3], traj_temp[:, 4])
      #  plt.axhline(y=0)
      #  plt.axhline(y=height)
      #  plt.axvline(x=0)
      #  plt.axvline(x=width)
      #  plt.title("Is bad : " + str(is_bad))
      #  plt.savefig('./test/hw.png')
    ...
    #  plt.axvline(x=width)
    #  plt.title("Is bad : " + str(is_bad))
    #  plt.savefig('./test/isbad.png')
    #else: 
    #  fig, axs = plt.subplots(5)
    #  axs[0].plot(traj_temp[:, 6], label='EOT')
    #  axs[0].plot(traj_temp[:, 8], label='HitWall')


    #  axs[1].plot(traj_temp[:, 0], marker='x', ms=1.5, linewidth=0.5)
    #  axs[2].plot(traj_temp[:, 1], marker='x', ms=1.5, linewidth=0.5)
    #  axs[3].plot(traj_temp[:, 2], marker='x', ms=1.5, linewidth=0.5)
    #  plt.title("Is bad : " + str(is_bad))
    #  plt.savefig('./test/good_1d.png')

    #  fig, axs = plt.subplots(1, 2)
    #  axs[0].plot(traj_temp[:, 3], traj_temp[:, 4])
    #  axs[0].axhline(y=0)
    #  axs[0].axhline(y=height)
    #  axs[0].axvline(x=0)
    #  axs[0].axvline(x=width)
    #  axs[1].plot(traj_temp[:, 3], traj_temp[:, 4])
    #  plt.title("Is bad : " + str(is_bad))
    #  plt.savefig('./test/good_scr.png')

    #  fig = go.Figure(data=[go.Scatter3d(x=traj_temp[:, 0].reshape(-1), y=traj_temp[:, 1].reshape(-1), z=traj_temp[:, 2].reshape(-1),
    #                                    mode='markers')])
  ...
  print("Final shape : ", trajectory.shape, ", each traj : ", trajectory[0].shape)
  len_traj = []
  # for idx in range(trajectory.shape[0]):
...
def latent_equality(data):
  ...
  for i in tqdm.tqdm(range(data.shape[0])):
    # Trajectory
    trajectory = data[i]
    trajectory_temp = data[i]
    x, y, z = trajectory_temp[:, 0], trajectory_temp[:, 1], trajectory_temp[:, 2]
    # Delta
    dx = (x[1:] - x[:-1]).astype(np.float64)
    dz = (z[1:] - z[:-1]).astype(np.float64)
    ...
    # print(dz_norm)
    # Latent
    sin, cos = trajectory[:, 18], trajectory[:, 19]
    #print(sin, cos)
    #print(np.sqrt((sin**2).astype(np.float64) + (cos**2).astype(np.float64)))
    #print((sin**2).astype(np.float64) + (cos**2).astype(np.float64))
    #exit()
    # Degree
    ...
    #if (not np.all(np.abs(np.mean(cos[1:]) - np.mean(dx_norm)) < tol)) or (not np.all(np.abs(np.mean(sin[1:]) - np.mean(dz_norm)) < tol)):
    if degree_diff > 1.0:
      fail = 1
      '''
      print("#"*50)
...
def vis(i, data):
  ...
  for i in range(n_latent):
    ...
    sin, cos = trajectory[:, 18], trajectory[:, 19]
    print("===>Sample  :", sin[0], cos[0], sin[0]**2 + cos[0]**2)
  ...
  for i in tqdm.tqdm(range(len(dataset_folder)), desc="Loading dataset"):
    ...
    for traj_type in trajectory_type:
      if os.path.isfile(dataset_folder[i] + "/{}Trajectory_Trial{}.csv".format(traj_type, trial_index[i])):
        trajectory_df[traj_type] = pd.read_csv(dataset_folder[i] + "/{}Trajectory_Trial{}.csv".format(traj_type, trial_index[i]), skiprows=[1], delimiter=',')
        if args.f_norm:
